...
const UserCard = ({ user, btn, status }) => {
    ...
    return (
        <div className={styles.main}>
            <div className={styles.cover} style={{ backgroundImage: `url(${user.bio.cover})` }}>
            ...
            <div className={styles.buttons}>
                {btn && user.id !== profile.id &&
                    <button onClick={() => friendShipREQ(user.id, profile.id)} disabled={disabled}>{
                        profile.friendreq.filter((item) => item.id === user.id).length > 0 ? <p><span><TiUserAdd /></span>Richiesta Inviata</p> :
                            profile.friends.filter((item) => item.id === user.id).length > 0 ? <p><span><TiTick /></span>Già Amico</p> : <p><span><TiUserAdd /></span>Invia Richiesta</p>
                    }</button>}
                {user.id !== profile.id && <Link to={'/Feisbrut-React-Social/discussion'} state={{ user: user }}><button><p><span><TiMail /></span>Messaggio</p></button></Link>}
            </div>
        </div>
    )
}
